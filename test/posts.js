...
describe('/posts route', () => {
    ...
    it('PUT /posts/:id', async () => {
        ...
        const res = await request.put(`posts/${postId}`).set('Authorization', `Bearer ${token}`).send(data);

        expect(res.body.data.title).to.eq(data.title);
        expect(res.body.data.body).to.eq(data.body);
    });
    it('DELETE /posts/:id', async () => {
        const res = await request.delete(`posts/${postId}`).set('Authorization', `Bearer ${token}`);
